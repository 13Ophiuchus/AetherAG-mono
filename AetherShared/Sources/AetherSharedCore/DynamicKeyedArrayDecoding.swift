...
public extension KeyedDecodingContainer where Key == DynamicCodingKeys {
    ...
    func decodeDynamicKeyedArray<Element: Decodable>(
        ofElement _: Element.Type,
        forKey key: DynamicCodingKeys
    ) throws -> [KeyedArrayGroup<Element>] {
        let nested = try nestedContainer(keyedBy: DynamicCodingKeys.self, forKey: key)
